...
> **MCP server that checks the live status of 3040 software services in real time.** Ask your AI agent "is GitHub down?" or "what's wrong with Sentry?" — and get a live answer pulled directly from official status pages, including full incident detail when something is broken.
...
StatusCraft gives your AI client 5 tools that fetch live status from **3040 major services**:

| Tool | What it does |
|---|---|
| `get_status` | Check one service — returns normalized status + incident detail when non-operational |
| `get_all_status` | Check all 3040 services at once, grouped by status (cached 60s) |
