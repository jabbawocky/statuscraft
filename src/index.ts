...
async function fetchHerokuStatus(svc: ServiceConfig): Promise<ServiceStatus> {
  const now = new Date().toISOString();
  try {
    const res = await fetch(svc.status_url, {
      signal: AbortSignal.timeout(8000),
    ...
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = (await res.json()) as { status?: Array<{ system: string; status: string }>; incidents?: unknown[] };
    ...
    if (hasRed) status = "major_outage";
    else if (hasYellow || incidents.length > 0) status = "degraded";
...
async function fetchStatusIOStatus(svc: ServiceConfig): Promise<ServiceStatus> {
  const now = new Date().toISOString();
  try {
    const res = await fetch(svc.status_url, {
      signal: AbortSignal.timeout(8000),
    ...
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = (await res.json()) as { result?: { status_overall?: { status?: string; status_code?: number } } };
    const code = data?.result?.status_overall?.status_code ?? 0;
    const statusText = data?.result?.status_overall?.status ?? "Unknown";
    let status: StatusIndicator;
    if (code === 100) status = "operational";
    else if (code === 200 || code === 300) status = "degraded";
    else if (code === 400) status = "partial_outage";
    else if (code >= 500) status = "major_outage";
    else status = "unknown";
...
async function fetchBetterStackStatus(svc: ServiceConfig): Promise<ServiceStatus> {
  ...
  try {
    ...
    switch (pageStatus.toUpperCase()) {
      case "UP":
      case "ALLGOOD": status = "operational"; break;
      case "DEGRADED":
      case "ALLDEGRADEDPERFORMANCE":
      case "HASISSUES":
      case "ONEPARTIALOUTAGE": status = "degraded"; break;
      case "DOWN":
      case "SOMEMAJOROUTAGE": status = "major_outage"; break;
      case "MAINTENANCE":
      case "UNDERMAINTENANCE":
      case "SOMEUNDERMAINTENANCE": status = "maintenance"; break;
      ...
      inputSchema: {
        ...
        properties: {
          search: {
            type: "string",
            description:
              "Optional name/ID search query. Filters services whose name or ID contains this string (case-insensitive). E.g. 'stripe', 'amazon', 'cloud'. Use this to find a service when you don't know its exact ID.",
          },
          filter_tag: {
            type: "string",
            description:
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          service: {
            type: "string",
            description: "Optional: service ID to refresh (e.g. 'github'). If omitted, refreshes all 3405 services.",
          },
        },
        required: [],
      },
  ...
  if (name === "list_services") {
    ...
    if (searchQuery) {
      filtered = filtered.filter(
        (s) => s.id.includes(searchQuery) || s.name.toLowerCase().includes(searchQuery)
      );
    }
    if (filterTag) {
    ...
    if (searchQuery) headerParts.push(`matching "${searchQuery}"`);
    if (filterTag) headerParts.push(`tagged "${filterTag}"`);
